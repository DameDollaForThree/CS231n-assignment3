...
def gram_matrix(features, normalize=True):
    ...
    # change to new shape
    features = features.view(N,C,H*W)    # torch.view is similar to np.reshape
    
    # preform batch matrix-matrix product
    # torch.transpose(input, dim0, dim1): dim0 and dim1 are swapped 
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for i in range(len(style_layers)):
        curr_layer = style_layers[i]
        curr_feat = feats[curr_layer]
        curr_gram = gram_matrix(curr_feat)
        curr_target = style_targets[i]
        curr_weight = style_weights[i]
        style_loss += curr_weight * torch.sum((curr_gram - curr_target)**2)
...
def tv_loss(img, tv_weight):
    ...
    loss = tv_weight * (torch.sum(row_diff**2) + torch.sum(column_diff**2))
